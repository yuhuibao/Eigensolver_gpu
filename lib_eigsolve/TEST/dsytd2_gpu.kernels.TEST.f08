...
function test_launch_dsytd2_gpu()
  ...
  ! TODO run the test
  CALL launch_dsytd2_gpu(0,c_null_ptr,grid,block,sharedMem,stream,lda,a,a_n1,a_n2,a_lb1,a_lb2,tau,tau_n1,tau_lb1,d,d_n1,d_lb1,e,e_n1,e_lb1,n) ! Modify sharedMem if other than default 0
  CALL hipCheck(hipDeviceSynchronize())
  CALL launch_dsytd2_gpu_cpu(0,c_null_ptr,grid,block,sharedMem,stream,lda,a,a_n1,a_n2,a_lb1,a_lb2,tau,tau_n1,tau_lb1,d,d_n1,d_lb1,e,e_n1,e_lb1,n)
...
program test_dsytd2_gpu_kernels
  implicit none
  integer :: globalErrorCode = 0, errorCode, fails = 0, tests = 0
  ! declare test functions and return type
  integer :: test_launch_dsytd2_gpu
  write(*,*) "SUITE test_dsytd2_gpu_kernels run ..."
  errorCode = test_launch_dsytd2_gpu()
  IF (errorCode > 0) THEN
    fails = fails + 1
    write(*,*) "TEST test_launch_dsytd2_gpu ... FAILURE"
  ELSE 
    write(*,*) "TEST test_launch_dsytd2_gpu ... SUCCESS"
  END IF
  tests = tests + 1
  globalErrorCode = globalErrorCode + errorCode

  IF (globalErrorCode > 0) THEN
    write(*,*) "SUITE test_dsytd2_gpu_kernels ... FAILURE passed:",(tests-fails)," failed:",fails," total:",tests
  ELSE 
    write(*,*) "SUITE test_dsytd2_gpu_kernels ... SUCCESS passed:",(tests-fails)," failed:",fails," total:",tests
  END IF
end program test_dsytd2_gpu_kernels