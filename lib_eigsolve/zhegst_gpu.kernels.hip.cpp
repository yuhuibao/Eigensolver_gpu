...
__global__ void krnl_185097_0(TODO declaration not found a, int k, int kb) {

  unsigned int j = 1 + threadIdx.y + blockIdx.y * blockDim.y;
  unsigned int i = 1 + threadIdx.x + blockIdx.x * blockDim.x;
  if ((j <= (k + kb - 1)) && (i <= (k + kb - 1))) {
    if ((j < i)) {
      a[_idx_a(i, j)] = ASTConjugate : {
        'match' : '        ! kernel do(2) <<<*,*, 0, stream1>>> do j = k, k + kb -
            1 do i = k,
                 k + kb - 1 if (j < i) then a(i, j) = conjg(a(j, i)) endif
                     end do end do ', ' location ': 150, ' ref ': <fort2hip.translator.ASTArithmeticExpression object at 0x7f35364ea828>, ' kind': None};
      }
    }
  }

  extern "C" void
  launch_krnl_185097_0(dim3 * grid, dim3 * block, const int sharedMem, hipStream_t stream, TODO declaration not found a, int k, int kb) {
    hipLaunchKernelGGL((krnl_185097_0), *grid, *block, sharedMem, stream, a, k, kb);
  }
  extern "C" void launch_krnl_185097_0_auto(const int sharedMem, hipStream_t stream, TODO declaration not found a, int k, int kb) {
    const unsigned int krnl_185097_0_NX = (k + kb - 1);
    const unsigned int krnl_185097_0_NY = (k + kb - 1);
  ...
  */
  ...
  __global__ void krnl_9c3ecb_1(TODO declaration not found a, int k, int kb) {
  ...
  extern "C" void
  launch_krnl_9c3ecb_1(dim3 * grid, dim3 * block, const int sharedMem, hipStream_t stream, TODO declaration not found a, int k, int kb) {
    hipLaunchKernelGGL((krnl_9c3ecb_1), *grid, *block, sharedMem, stream, a, k, kb);
  }
  extern "C" void launch_krnl_9c3ecb_1_auto(const int sharedMem, hipStream_t stream, TODO declaration not found a, int k, int kb) {
    const unsigned int krnl_9c3ecb_1_NX = (k + kb - 1);
    const unsigned int krnl_9c3ecb_1_NY = (k + kb - 1);
