...
__global__ void krnl_afb01f_0(int kb, TODO declaration not found a, int k) {
...
extern "C" void
launch_krnl_afb01f_0(dim3 *grid, dim3 *block, const int sharedMem, hipStream_t stream, int kb, TODO declaration not found a, int k) {
  hipLaunchKernelGGL((krnl_afb01f_0), *grid, *block, sharedMem, stream, kb, a, k);
}
extern "C" void launch_krnl_afb01f_0_auto(const int sharedMem, hipStream_t stream, int kb, TODO declaration not found a, int k) {
  const unsigned int krnl_afb01f_0_NX = (k + kb - 1);
  const unsigned int krnl_afb01f_0_NY = (k + kb - 1);
