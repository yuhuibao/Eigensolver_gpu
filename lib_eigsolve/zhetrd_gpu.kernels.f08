...
module zhetrd_gpu_kernels
  ...
  interface
    ...
    subroutine launch_zher2_mv_kernel(grid,&
        ...
        x,&
        x_n1,&
        x_lb1) bind(c, name="launch_zher2_mv_kernel")
      use iso_c_binding
      use hip
      implicit none
      type(dim3,,intent(IN, :: grid
    ...
    subroutine launch_stacked_zgemv_n(grid,&
        ...
        y,&
        y_n1,&
        y_lb1) bind(c, name="launch_stacked_zgemv_n")
      use iso_c_binding
      use hip
      implicit none
      type(dim3,,intent(IN, :: grid
  ...
  contains
    ...
    subroutine launch_krnl_9c27cb_1_cpu(sharedMem,&
      ...
      INTEGER(kind=),value :: n
      INTEGER(kind=) :: k
            complex(8), 1:nb),target :: w(w_n1,w_n2)
    ...
    subroutine launch_zher2_mv_kernel_cpu(n,&
      ...
      integer(c_int),value,intent(IN) :: x_n1
      integer(c_int),value,intent(IN) :: x_lb1
            complex(8), 1:M), intent(in) ,target :: v(v_n1,v_n2)
            complex(8), 1:M), intent(in) ,target :: w(w_n1,w_n2)
            complex(8), 2),target :: w2(w2_n1,w2_n2)
    ...
    subroutine launch_zher2_mv_zlarfg_kernel_cpu(n,&
      ...
      INTEGER(kind=),value :: finished
            ...
            complex(8), 2),target :: w2(w2_n1,w2_n2)
            real(8),target :: x(x_n1)
            complex(8),target :: x2(x2_n1)
      CALL hipCheck(hipMemcpy(c_loc(v),_v,C_SIZEOF(v),hipMemcpyDeviceToHost))
      CALL hipCheck(hipMemcpy(c_loc(w),_w,C_SIZEOF(w),hipMemcpyDeviceToHost))
      CALL hipCheck(hipMemcpy(c_loc(w2),_w2,C_SIZEOF(w2),hipMemcpyDeviceToHost))
      CALL hipCheck(hipMemcpy(c_loc(x),_x,C_SIZEOF(x),hipMemcpyDeviceToHost))
      CALL hipCheck(hipMemcpy(c_loc(x2),_x2,C_SIZEOF(x2),hipMemcpyDeviceToHost))
            implicit none
            integer, value                                        :: N, M, ldv, ldw, ldw2
            complex(8), dimension(1:ldv, 1:M), device, intent(in) :: V
            complex(8), dimension(1:ldw, 1:M), device, intent(in) :: W
      ...
      CALL hipCheck(hipMemcpy(_w2,c_loc(w2),C_SIZEOF(w2),hipMemcpyHostToDevice))
      CALL hipCheck(hipMemcpy(_x,c_loc(x),C_SIZEOF(x),hipMemcpyHostToDevice))
      CALL hipCheck(hipMemcpy(_x2,c_loc(x2),C_SIZEOF(x2),hipMemcpyHostToDevice))
    ...
    subroutine launch_stacked_zgemv_c_cpu(m,&
      ...
      integer(c_int),value,intent(IN) :: z2_n1
      integer(c_int),value,intent(IN) :: z2_lb1
            complex(8), M), intent(in)  ,target :: v(v_n1,v_n2)
            complex(8), M), intent(in)  ,target :: w(w_n1,w_n2)
            complex(8), intent(in)       ,target :: x(x_n1)
    ...
    subroutine launch_stacked_zgemv_n_cpu(m,&
      ...
      integer(c_int),value,intent(IN) :: y_n1
      integer(c_int),value,intent(IN) :: y_lb1
            complex(8), N), intent(in)  ,target :: v(v_n1,v_n2)
            complex(8), N), intent(in)  ,target :: w(w_n1,w_n2)
            complex(8), intent(in)       ,target :: z1(z1_n1)
            complex(8), intent(in)       ,target :: z2(z2_n1)
