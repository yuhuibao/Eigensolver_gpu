...
__global__ void dsymv_gpu(int n, int lda, double *a, double *x, double *y) {
#undef _idx
#undef _idx_a
#undef _idx_ar_s
#define _idx(a) ((a - 1))
#define _idx_a(a, b) ((a - 1) + lda * (b - 1))
#define _idx_ar_s(a, b) ((a - 1) + (BX + 1) * (b - 1))
  ...
  // ! Loop over columns (skip all lower triangular blocks)
  for (int jj = (ii + (blockIdx.x) * blockDim.x); jj <= n;
       jj += (gridDim.x * blockDim.x)) {
    j = (jj + ty - 1);
    // ! Load block into shared memory
    // ! CASE 1: Diagonal block
    if (ii == jj) {
      ...
      for (int k = 0; k <= (NTILES - 1); k += 1) {
        if ((i <= n && (j + k * blockDim.y) <= n)) {
          ar_s[_idx_ar_s(tx, (ty + k * blockDim.y))] =
              a[_idx_a(i, (j + k * blockDim.y))];
        }
      }
      __syncthreads(); // ! Reflect to populate lower triangular part with true
                       // values of A
      for (int k = 0; k <= (NTILES - 1); k += 1) {
        if ((tx > (ty + k * blockDim.y))) {
          ar_s[_idx_ar_s(tx, (ty + k * blockDim.y))] =
              ar_s[_idx_ar_s((ty + k * blockDim.y), tx)];
        }
      }
      __syncthreads();
      for (int k = 0; k <= (NTILES - 1); k += 1) {
        if ((i <= n && (j + k * blockDim.y) <= n)) {
          mysum = (mysum + ar_s[_idx_ar_s(tx, (ty + k * blockDim.y))] *
                               x[_idx((j + k * blockDim.y))]);
        }
      ...
      if ((ty == 1 && (jj + tx - 1) <= n)) {
        istat = atomicAdd(y+_idx((jj + tx - 1)), r_s[_idx(tx)]);
      }
      // !call __syncthreads()
    }
    __syncthreads();
...
extern "C" void launch_dsymv_gpu(dim3 *grid, dim3 *block, const int sharedMem,
                                 hipStream_t stream, int n, int lda, double *a,
                                 double *x, double *y) {
  hipLaunchKernelGGL((dsymv_gpu), *grid, *block, sharedMem, stream, n, lda, a,
                     x, y);
}
// END dsymv_gpu
